...
bigger gaussian is a parameter $a$.
Finally
...
\begin{eqnarray}
\rho(x, y) &=&
\frac{f(1-v)}{2\pi a^2}\exp\left[-\frac{1}{2a^2}\left(qx^2 + y^2/q\right)\right]
+\frac{fv}{2\pi (ua)^2}\exp\left[-\frac{1}{2(ua)^2}\left(qx^2 + y^2/q\right)\right]
\end{eqnarray}
...
GalaxyField data.
{\bf Bottom panel: }The posterior weights of the saved samples. Note the phase
transition at $\log(X) \approx -100$ nats. While this doesn't affect the
