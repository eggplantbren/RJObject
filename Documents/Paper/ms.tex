...
\maketitle
\abstract{Blah...
We demonstrate the techniques on a seemingly simple problem of detecting
sinusoidal signals in noise, which exhibits a phase transition.}
...
For the interim prior for the total fluxes of the galaxies, we use the Pareto
distribution. The probability density function is
\begin{eqnarray}
p(f | f_{\rm min}, \alpha) &=&
\left\{
\begin{array}{lr}
\frac{\alpha f_{\rm min}^{\alpha}}{f^{\alpha + 1}}, & f \geq f_{\rm min}\\
